...
4b. FAULT-TOLERANT DISPATCH per docs/plugin-isolation.md layers 1–3 (this is a hard requirement, not polish): every contract/hook invocation goes through a single HookDispatcher that (a) wraps in try/catch(\Throwable), returns the contract's declared safe default, and records the failure (plugin, hook, exception) to the audit log; (b) circuit-breaks — 5 failures of one plugin within 10 minutes auto-disables it with an audit entry; (c) records "plugin X in hook Y" before invocation and registers a shutdown handler that attributes fatals (error_get_last) to the in-flight plugin, feeding the same breaker; (d) supports MAGNA_SAFE_MODE=1 (env or storage flag file) booting with all plugins disabled; (e) runs hooks declared "execution": "queued" in magna.json as queued jobs instead of inline. Tests: a plugin whose hook throws still renders the page minus its contribution + failure recorded; 5 throws auto-disable; safe mode boots with plugins off; queued-declared hook dispatches a job.
